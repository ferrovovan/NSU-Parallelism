...
def fun_thread_read(
    path_video: str, frame_queue: queue.Queue, event_stop: threading.Event
):
...
def fun_thread_safe_predict(
    frame_queue: queue.Queue, out_queue: queue.Queue, event_stop: threading.Event
):
    model = YOLO(
        model="yolov8s-pose.pt", verbose=False
    ...
    while True:
        try:
            ...
            results = model.predict(source=frame, device="cpu")[
                0
            ].plot()  # Обнаруживаем объекты на кадре
...
def main(arg):
    ...
    frame_queue = queue.Queue(10000)  # Очередь для кадров
    ...
    for _ in range(arg.th):
        threads.append(
            threading.Thread(
                target=fun_thread_safe_predict,
                args=(
                    frame_queue,
                    out_queue,
                    event_stop,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--in", type=str, default="Videos/input.mp4", help="input/path/to/video.mp4"
    )
    parser.add_argument(
        "--out", type=str, default="Videos/result.mp4", help="output/path/to/result.mp4"
    )
